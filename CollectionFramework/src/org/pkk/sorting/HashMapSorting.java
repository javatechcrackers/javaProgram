...
/**
 * * How to sort HashMap in Java by keys and values. * HashMap doesn't guarantee
 * any order, so you cannot rely on it, even if * it appear that it storing
 * entries in a particular order, because * it may not be available in future
 * version e.g. earlier HashMap stores * integer keys on the order they are
 * inserted but from Java 8 it has changed. * * @author WINDOWS 8
 */
public class HashMapSorting {
	public static void main(String args[]) throws ParseException { // let's
																	// create a
																	// map with
																	// Java
																	// releases
		...
		} // Now let's sort HashMap by keys first // all you need to do is
			// create a TreeMap with mappings of HashMap // TreeMap keeps all
		...
		// Now let's sort the HashMap by values // there is no direct way to
			// sort HashMap by values but you // can do this by writing your own
			// comparator, which takes // Map.Entry object and arrange them in
			// order increasing // or decreasing by values.
